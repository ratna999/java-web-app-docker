      ...
      stage('Run Docker Image In Dev Server'){
        ...
        def dockerRun = ' docker run  -d -p 8080:8080 --name java-web-app 14551a0586/java-web-app'
         ...
         sshagent(['DOCKER_SERVER']) {
          sh 'ssh -o StrictHostKeyChecking=no ec2-user@172.31.6.209 docker stop java-web-app || true'
          sh 'ssh  ec2-user@172.31.6.209 docker rm java-web-app || true'
          sh 'ssh  ec2-user@172.31.6.209 docker rmi -f  $(docker images -q) || true'
          sh "ssh  ec2-user@172.31.6.209 ${dockerRun}"
       }
